...
user_fields = {
    ...
    'uri': fields.String(attribute=lambda x: url_for(
        'api.user', user_id=x.id, _external=True)),
}
user_list = {
...
tag_fields = {
    ...
    'uri': fields.String(attribute=lambda x: url_for('api.tag', tag_id=x.id,
                                                     _external=True)),
}
tag_list = {
...
book_fields = {
    ...
    'can_borrow_number':
        fields.Integer(attribute=lambda x: x.can_borrow_number()),
    'tags': fields.List(fields.Nested(tag_fields), attribute=lambda x: x.tags),
    'uri': fields.String(attribute=lambda x: url_for('api.book', book_id=x.id,
                                                     _external=True)),
}
book_list = {
...
logs_info_fields = {
    ...
    'uri': fields.String(attribute=lambda x: url_for('api.log', log_id=x.id,
                                                     _external=True)),
}
logs_info_list = {
...
comment_fields = {
    ...
    'uri': fields.String(attribute=lambda x: url_for(
        'api.comment', comment_id=x.id, _external=True)),
}
comment_list = {
        ...
        'borrowing_logs': fields.Nested(
            {
                ...
                'next': fields.String(
                    attribute=lambda this: url_for(
                        'api.loglist', user_id=this.items[0].book_id,
                        returned=0, page=2, count=default_per_page,
                ...
                Log.id.desc()).paginate(page=1, per_page=default_per_page)),
        ...
        'borrowed_logs': fields.Nested(
            {
                ...
                'next': fields.String(
                    attribute=lambda this: url_for(
                        'api.loglist', user_id=this.items[0].book_id,
                        returned=1, page=2, per_page=default_per_page,
                        _external=True) if this.has_next else None),
                'prev': fields.String(attribute=''),
                'total': fields.Integer,
                'pages_count': fields.Integer(attribute='pages'),
        ...
        'comments': fields.Nested(
            {
                ...
                'next': fields.String(
                    attribute=lambda this: url_for(
                        'api.commentlist', user_id=this.items[0].book_id,
                        deleted=0, page=2, per_page=default_per_page,
                        _external=True) if this.has_next else None),
                'prev': fields.String(attribute=''),
                'total': fields.Integer,
                'pages_count': fields.Integer(attribute='pages'),
            ...
            'borrowing_logs': fields.Nested(
                {
                    ...
                    'next': fields.String(
                        attribute=lambda this: url_for(
                            'api.loglist', book_id=this.items[0].book_id,
                            returned=0, page=2,
                    ...
                    'current_page': fields.Integer(default=1),
                    'per_page': fields.Integer,
                }, attribute=lambda this: this.logs.filter_by(
                    returned=0).order_by(Log.id.desc()).paginate(
                    page=1, per_page=default_per_page)),
            ...
            'borrowed_logs': fields.Nested(
                {
                    ...
                    returned=1).order_by(Log.id.desc()).paginate(
                    page=1, per_page=default_per_page)),
            ...
            'comments': fields.Nested(
                {
                    ...
                    'next': fields.String(
                        attribute=lambda this: url_for(
                            'api.commentlist', book_id=this.items[0].book_id,
                            deleted=0, page=2, per_page=default_per_page,
                            _external=True) if this.has_next else None),
                    'prev': fields.String(attribute=''),
                    'total': fields.Integer,
                ...
                attribute=lambda this: this.comments.filter_by(
                    deleted=0).order_by(
                    Comment.id.desc()).paginate(
                    page=1, per_page=default_per_page)),
...
logs_info_detail_fields = dict(logs_info_fields, **{
    'user': fields.Nested(user_fields, attribute=lambda x: x.user),
    'book': fields.Nested(book_fields, attribute=lambda x: x.book)})

tag_detail_fields = dict(tag_fields, **{
    'books': fields.List(fields.Nested(book_fields),
                         attribute=lambda x: x.books)})
