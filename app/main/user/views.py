...
def detail(user_id):
    ...
    return render_template("user_detail.html", user=the_user, logs=logs,
...
def edit(user_id):
    if current_user.id == user_id or current_user.can(
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(u'data update successfully!', "info")
        ...
        return render_template('user_edit.html', form=form, user=the_user,
                               title=u"Edit data")
    else:
        abort(403)
    ...
    if current_user.id == user_id or current_user.can(
        ...
        if avatar_upload_form.validate_on_submit():
            if'avatar' in request.files:
                ...
                avatar_name = avatars.save(avatar_upload_form.avatar.data,
                                           folder=forder)
                the_user.avatar = json.dumps({"use_out_url": False,
                                              "url": avatar_name})
                db.session.add(the_user)
                db.session.commit()
        ...
        if avatar_edit_form.validate_on_submit():
            the_user.avatar = json.dumps(
                {"use_out_url": True,
                 "url": avatar_edit_form.avatar_url.data})
            db.session.add(the_user)
            db.session.commit()
            return redirect(url_for('user.detail', user_id=user_id))
